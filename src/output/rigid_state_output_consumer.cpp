...
std::unique_ptr<::state_machine::runtime::Task<ros::NodeHandle>> makePublishStateTask(
    ros::Publisher state_pub, estimator_rigid_state::RigidStateEstimate state_msg) {
...
RigidStateOutputConsumer::RigidStateOutputConsumer(
    ...
    RigidStateEstimatorRuntime& runtime, std::string state_topic, std::string vision_pose_topic,
    uint32_t queue_size)
    : executor_(executor), runtime_(runtime) {
    state_pub_ =
        nh.advertise<estimator_rigid_state::RigidStateEstimate>(std::move(state_topic), queue_size);
    vision_pose_pub_ =
...
bool RigidStateOutputConsumer::handle(const ::state_machine::Event& event) {
    ...
    if (event.id == output_event_type::PUBLISH_STATE) {
        const RigidStateEstimatorOutput output = runtime_.refreshOutputSnapshot();
    ...
    if (event.id == output_event_type::PUBLISH_VISION_POSE) {
        const RigidStateEstimatorOutput output = runtime_.snapshotOutput();
...
estimator_rigid_state::RigidStateEstimate RigidStateOutputConsumer::makeStateMessage(
    const RigidStateEstimatorOutput& output, const ros::Time& stamp) {
    estimator_rigid_state::RigidStateEstimate msg;
...
bool RigidStateOutputConsumer::canPublishVisionPose(const RigidStateEstimatorOutput& output) {
