...
double yawFromQuaternion(const geometry_msgs::Quaternion& value) {
    ...
    if (!std::isfinite(q.w()) || !std::isfinite(q.x()) || !std::isfinite(q.y()) ||
        !std::isfinite(q.z()) || q.norm() <= 1.0e-9) {
...
void PlanarStateInputProducer::imuCallback(const sensor_msgs::Imu::ConstPtr& msg) {
    ...
    sample.valid = isFinite(sample.angular_velocity_z) && sample.linear_acceleration.allFinite();
...
ros::Time PlanarStateInputProducer::messageStampOrNow(const ros::Time& stamp) {
    return stamp.isZero() ? ros::Time::now() : stamp;
}

void PlanarStateInputProducer::updateImuPeriod(xgc2_math::PlanarInertialSample& sample,
                                               double stamp_sec) {
...
void PlanarStateInputProducer::updatePosePeriod(xgc2_math::PlanarPoseMeasurement& sample,
                                                double stamp_sec) {
    const bool has_prev = sample.received && std::isfinite(sample.stamp_sec);
    const double raw_dt_sec = has_prev ? stamp_sec - sample.stamp_sec : 0.0;
    const bool finite_dt = std::isfinite(raw_dt_sec);
