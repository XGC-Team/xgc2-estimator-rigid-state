...
class RigidStateInputProducer {
   public:
    using EventSink = std::function<::state_machine::Status(::state_machine::Event,
                                                            const RigidStateEstimatorInput&)>;
   ...
   private:
    ...
    static void updateImuPeriod(xgc2_observer::InertialSample& sample, double stamp_sec);
    static void updatePosePeriod(xgc2_observer::PoseMeasurement& sample, double stamp_sec);

    EventSink event_sink_;
    RigidStateEstimatorInput runtime_input_{};
