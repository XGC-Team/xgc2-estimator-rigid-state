...
namespace estimator_rigid_state::health_checks {
...
inline ::state_machine::EventId transitionEventFor(::state_machine::StateId state) {
    switch (state) {
        ...
        case state_type::Initializing:
            return event_type::HEALTH_TO_INITIALIZING;
...
inline HealthStatus classify(const RigidStateEstimatorInput& input,
                             const RigidStateEstimatorConfig& config, bool initialized,
    ...
    if (fault_requested) {
        ...
        health.state = imu_ready && vrpn_ready ? state_type::Initializing : state_type::SelfCheck;
    } else if (!imu_ready) {
        health.state = state_type::Fault;
    } else if (vrpn_ready) {
