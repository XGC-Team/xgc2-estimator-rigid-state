...
::state_machine::EventId eventForCondition(HealthCondition condition) {
    switch (condition) {
        ...
        case HealthCondition::kInitializationReady:
            return event_type::HEALTH_INITIALIZATION_READY;
    ...
    throw std::runtime_error("unknown VRPN PX4 rotor health condition: " +
...
void HealthMonitorState::evaluateAndPostTransition(::state_machine::StateContext& ctx,
    ...
    if (!status.ok()) {
        throw std::runtime_error("post VRPN PX4 rotor health event: " + status.message);
    }
}
